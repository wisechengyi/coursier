...
object Fetch {
  ...
  def find[F[_]](
    ...
    val task0 =
      lookups.foldLeft[F[Either[Seq[String], (Artifact.Source, Project)]]](F.point(Left(Nil))) {
    ...
    val task =
      F.map(task0)(e => e.left.map(_.reverse): Either[Seq[String], (Artifact.Source, Project)])
    EitherT(task)
  }

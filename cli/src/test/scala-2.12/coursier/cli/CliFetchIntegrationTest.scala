...
class CliFetchIntegrationTest extends FlatSpec with CliTestLib with Matchers {
  ...
  "scalafix-cli fetch" should "discover all main classes" in {
    val fetchOpt = FetchOptions(common = CommonOptions())
    val fetch =
      Fetch(fetchOpt, RemainingArgs(Seq("ch.epfl.scala:scalafix-cli_2.12.4:0.5.10"), Seq()))
    Helper.mainClasses(fetch.helper.loader) should contain theSameElementsAs Map(
      ("", "") -> "com.martiansoftware.nailgun.NGServer",
  ...
  "Module level" should "exclude correctly" in withFile("junit:junit--org.hamcrest:hamcrest-core") {
    (file, _) =>
      withFile() { (jsonFile, _) =>
        val commonOpt =
          CommonOptions(localExcludeFile = file.getAbsolutePath, jsonOutputFile = jsonFile.getPath)
   ...
   */
  "avro exclude xz" should "not fetch xz" in withFile("org.apache.avro:avro--org.tukaani:xz") {
    (file, writer) =>
      withFile() { (jsonFile, _) =>
        val commonOpt =
          CommonOptions(localExcludeFile = file.getAbsolutePath, jsonOutputFile = jsonFile.getPath)
    ...
    withFile() { (jsonFile, _) =>
      {
        ...
        val compressNode =
          node.dependencies.find(_.coord == "org.apache.commons:commons-compress:jar:tests:1.5")

        assert(compressNode.isDefined)
        compressNode.get.file
          .map(f => assert(f.contains("commons-compress-1.5-tests.jar")))
      ...
      withFile() { (jsonFile, _) =>
        {
          ...
          compressNodes.head.file
            .map(f => assert(f.contains("commons-compress-1.5.jar")))
            .orElse(fail("Not Defined"))

          assert(compressNodes.last.coord == "org.apache.commons:commons-compress:jar:tests:1.5")
          compressNodes.last.file
            .map(f => assert(f.contains("commons-compress-1.5-tests.jar")))
            .orElse(fail("Not Defined"))
        }
      }
    ...
    withFile() { (jsonFile, _) =>
      {
        val commonOpt = CommonOptions(
          ...
          intransitive = List("org.apache.commons:commons-compress:1.5")
        )
        val fetchOpt = FetchOptions(common = commonOpt)
    ...
    withFile() { (jsonFile, _) =>
      {
        ...
        val compressNode =
          node.dependencies.find(_.coord == "org.apache.commons:commons-compress:jar:tests:1.4.1")

        assert(compressNode.isDefined)
        compressNode.get.file
          .map(f => assert(f.contains("commons-compress-1.4.1-tests.jar")))
          .orElse(fail("Not Defined"))
        ...
        withFile("tada", "coursier-fetch-test", ".jar") { (testFile, _) =>
          {
            ...
            val commonOpt =
              CommonOptions(jsonOutputFile = jsonFile.getPath, cacheFileArtifacts = true)
            val fetchOpt = FetchOptions(common = commonOpt)
            ...
            assert(
              urlInJsonFile2.contains("coursier/cache") && urlInJsonFile2
  ...
  "external dep url" should "fetch junit-4.12.jar" in withFile() { (jsonFile, _) =>
    {
      ...
      val externalUrl =
        encode("http://central.maven.org/maven2/junit/junit/4.12/junit-4.12.jar", "UTF-8")
      ...
      depNodes.head.file
        .map(f => assert(f.contains("junit/junit/4.12/junit-4.12.jar")))
        .orElse(fail("Not Defined"))
    }
  }
    ...
    (jsonFile, _) =>
      {
        ...
        jacksonCoreNodes.head.file
          .map(
            f =>
              assert(
                f.contains("org/codehaus/jackson/jackson-core-asl/1.8.8/jackson-core-asl-1.8.8.jar")
            )
          )
          .orElse(fail("Not Defined"))
      }
  ...
  "Bad pom resolve" should "succeed with retry" in withTempDir("tmp_dir") { dir =>
    {
      def runFetchJunit() = {
        val fetchOpt = FetchOptions(
          common = CommonOptions(cacheOptions = CacheOptions(cache = dir.getAbsolutePath))
        )
        val fetch = Fetch(fetchOpt, RemainingArgs(Seq("junit:junit:4.12"), Seq()))
        assert(
          fetch.files0
        ...
        val junitJarPath =
          fetch.files0.map(_.getAbsolutePath()).filter(_.contains("junit-4.12.jar")).head
        ...
        def runFetchJunit() = {
          val fetchOpt = FetchOptions(
            common = CommonOptions(
              mode = "force",
              cacheOptions = CacheOptions(cache = dir.getAbsolutePath)
            )
          )
          ...
          val junitJarPath =
            fetch.files0.map(_.getAbsolutePath()).filter(_.contains("junit-4.6.jar")).head
          Paths.get(junitJarPath)
        }
